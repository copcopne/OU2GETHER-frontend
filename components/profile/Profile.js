                ...
                try {
                    ...
                    if (res.status === 200) {
                        if (res.data.number_of_followers > 0) {
                            const followers = await authApis(token).get(endpoints['getUserFollowers'](userId));
    ...
    const handleFollow = async () => {
        ...
        if (res.status === 200) {
            setProfileData({
                ...
                number_of_followers: profileData.is_following ? profileData.number_of_followers - 1 : profileData.number_of_followers + 1
            });
    ...
    return (
        <ScrollView style={ProfileStyle.container}>
            ...
            <View style={ProfileStyle.m}>
                <Text style={[ProfileStyle.name, ProfileStyle.m, { flexShrink: 1, flexWrap: 'wrap', maxWidth: '70%' }]}>{profileData?.last_name + " " + profileData?.first_name}</Text>
                <Text style={[ProfileStyle.m, ProfileStyle.username]}>{profileData?.username}</Text>
                {profileData?.bio ? <Text style={[ProfileStyle.bio, ProfileStyle.m]}>{profileData?.bio}</Text> : <></>}

                <TouchableOpacity style={[ProfileStyle.followerAvatarContainer, ProfileStyle.r]}>
                    {profileData.number_of_followers > 0 && minimalFollowers.map((follower, index) => {
                        if (index < 2) {
                            return (
                                <Image
                                    key={index}
                    ...
                    <Text style={[
                        ...
                        profileData.number_of_followers > 0
                            ? ProfileStyle.followersText : null
                    ]}
                    >
            ...
            <View style={[ProfileStyle.r, ProfileStyle.actions]}>
                {profileData?.is_myself ?
                    <TouchableOpacity
                        ...
                        <Text style={[ProfileStyle.buttonText, ProfileStyle.p]}>Chỉnh sửa thông tin cá nhân</Text>
                    </TouchableOpacity>
                    : <><TouchableOpacity
                        style={[ProfileStyle.button, { flex: 1, margin: 5 }]}
                        onPress={() => handleFollow()}>
                        <Text style={[ProfileStyle.buttonText, ProfileStyle.p]}>{profileData?.is_following ? "Bỏ theo dõi" : "Theo dõi"}</Text>
                    </TouchableOpacity>
                        <TouchableOpacity style={[ProfileStyle.button, { flex: 1, margin: 5 }]}>
                            <Text style={[ProfileStyle.buttonText, ProfileStyle.p]}>Nhắn tin</Text>
                        </TouchableOpacity>
                    </>
                }
            </View>
            {profileData.is_myself &&
                ...
                ><BottomSheetView style={{ flex: 1 }}>
                        <EditProfile profileData={profileData} modalRef={bottomSheetModalRef} />
                    </BottomSheetView>
                </BottomSheetModal>}
